...
<meta name="description" content="A list of every game you have played in the UKPL" />
<title>UK Pinball League – Your scores</title>
...
$stmt = mysqli_prepare($mysqli, "SELECT player, machine, score, meet FROM LeagueResults1 WHERE player=? ORDER BY machine, score DESC"); 

   mysqli_stmt_bind_param($stmt, "s", $player);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $player, $machine, $score, $meet);    
  

	
echo "<h1>$player's scores</h1>"; ?>
...
$newmachine = str_replace("Creature from the Black Lagoon", "Creature f/t Black Lagoon", $machine);
	...
	echo "<tr>\n
		...
		<td bgcolor='".$bgcolor."'><span style='white-space:nowrap'>$newmachine</span></td>\n
